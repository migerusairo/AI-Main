...
class TCUAdvisor(MDApp):
    def build(self):
        ...
        screen.add_widget(Builder.load_file("Love_command.kv"))

        screen.add_widget(Builder.load_file("Academic_message-screen.kv"))
        screen.add_widget(Builder.load_file("Academic_command.kv"))

        screen.add_widget(Builder.load_file("Family_message-screen.kv"))
        screen.add_widget(Builder.load_file("Family_command.kv"))
        return screen
    ...
    def send_love(self):
        ...
        if screen.get_screen('Love_message-screen').text_input != "":
            ...
            for i in data['intents']:
                if i['tag'] == tag:
                    screen.get_screen('Love_message-screen').chat_list.add_widget(
                        Response(text=np.random.choice(i['responses']), size_hint_x=.75, halign="justify"))
    ...
    def wish(self):
        ...
        if 0 <= hour < 12:
            screen.get_screen('Home-screen').add_widget(Wish(text="Good Morning"))
            return
        elif 12 <= hour < 18:
            screen.get_screen('Home-screen').add_widget(Wish(text="Good afternoon"))
            return
        else:
            screen.get_screen('Home-screen').add_widget(Wish(text="Good Evening"))
            return
